...
async def ask_for_code(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.message.edit_text("📖 Надішли код — поясню що він робить простими словами.")
    await state.set_state(ExplainStates.waiting_for_code)
    await callback.answer()


@router.message(ExplainStates.waiting_for_code)
async def handle_code(message: Message, state: FSMContext) -> None:
    ...
    result = await claude.explain_code(code, lang)
